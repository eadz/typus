    ...
    @items.each do |item|
      ...
      @model.typus_fields_for('list').each do |column|
        ...
        when 'boolean'
          ...
          html << "<td width=\"20px\" align=\"center\">#{link_to image, { :params => params.merge(:controller => 'typus', :model => model, :action => 'toggle', :field => column[0], :id => item.id) } , :confirm => "Change this #{@model.to_s.downcase} #{params[:field]}?"}</td>"
        when "datetime"
